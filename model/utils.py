...
def WarpImages(input_map, H, start, patch_size):
    ...
    goal = torch.cat([batch_x, batch_y, torch.ones_like(batch_x, dtype=torch.float).to(device)], -1).permute(0, 2, 1).contiguous()

    img_pt = torch.bmm(H, goal).permute(0, 2, 1).contiguous()
    ...
    flow = new_batch_xy - goal.permute(0, 2, 1)[:, :, :2].contiguous()
    offset_batch_xy = goal.permute(0, 2, 1)[:, :, :2].contiguous() + start.view(bs, 1, 2).contiguous() + flow
    new_batch_img = bilinear_interpote(input_map.permute(0, 2, 3, 1).view(bs, -1, imgc).contiguous(),
                                       offset_batch_xy[:, :, 0],
                                       offset_batch_xy[:, :, 1], patch_size, bs, imgh, imgw, imgc)
    new_batch_img = new_batch_img.permute(0, 3, 1, 2).contiguous()
    return new_batch_img, flow.view(bs, patch_size[1], patch_size[0], 2).contiguous()
...
def CropPatchFromFull(patch_size, full_img, start, rescale=False):
    ...
    batch_y = torch.repeat_interleave(torch.arange(ph), repeats=pw).view(1, -1, 1).repeat(bs, 1, 1).to(device) + \
              start[:, :, 1:].repeat(1, pw * ph, 1)
    if rescale:
        new_batch_img = bilinear_interpote(full_img.permute(0, 2, 3, 1).view(bs, -1, c).contiguous(),
                                           batch_x,
        ...
        full_img_flatten = full_img.permute(0, 2, 3, 1).reshape(-1, c).contiguous()
        dim1 = h * w
        dim2 = w

        base = torch.arange(0, bs, dtype=torch.int).to(device).view(-1, 1).contiguous()
        base = base * dim1
    ...
    return new_batch_img.reshape(bs, patch_size[1], patch_size[0], c).permute(0, 3, 1, 2).contiguous()
...
def get_point_pairs(src_p, divide):  # src_p: shape=(bs, 2, h, w)
    ...
    src_p = src_p.permute(0, 1, 3, 4, 2).reshape(bs, divide * divide, 4, 2).contiguous()
    return src_p
...
def transformer(I, vgrid, train=True):
    ...
    def _interpolate(im, x, y, out_size, scale_h):
        ...
        if im.is_cuda:
            base = torch.arange(0, num_batch).int().to(im.device)
        else:
            base = torch.arange(0, num_batch).int()

        base = base * dim1
        base = base.repeat_interleave(out_height * out_width, axis=0)  # 其实就是单纯标出batch中每个图的下标位置
...
def get_warp_flow(img, flow, start=0):
    ...
    grid_warp = get_grid(batch_size, patch_size_h, patch_size_w, start)[:, :2, :, :].to(flow.device) + flow
    img_warp = transformer(img, grid_warp)
    return img_warp


def upsample2d_flow_as(inputs, target_as, mode="bilinear", if_rate=False, align_corners=True):
    _, _, h, w = target_as.size()
    if if_rate:
        _, _, h_, w_ = inputs.size()
        inputs[:, 0, :, :] *= (w / w_)
        inputs[:, 1, :, :] *= (h / h_)
    res = F.interpolate(inputs, [h, w], mode=mode, align_corners=align_corners)
    return res

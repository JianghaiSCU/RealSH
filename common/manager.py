...
class Manager():
    ...
    def print_train_info(self):
        ...
        print_str += " sup: %.4f(%.4f)" % (self.loss_status['supervise'].val, self.loss_status['supervise'].avg)
        print_str += " photo: %.4f(%.4f)" % (self.loss_status['photo_loss'].val, self.loss_status['photo_loss'].avg)
        print_str += " score: %.4f(%.4f)" % (self.loss_status['score'].val, self.loss_status['score'].avg)
        print_str += " total: %.4f(%.4f)" % (self.loss_status['total'].val, self.loss_status['total'].avg)
